...
CREATE TABLE "orders" (
	...
	"checkout_state" text DEFAULT 'creating' NOT NULL,
	"checkout_lease_token" text,
	"checkout_lease_expires_at" timestamp with time zone,
	...
	CONSTRAINT "order_checkout_state_valid" CHECK ("orders"."checkout_state" in ('creating','created','failed'))
);
--> statement-breakpoint
...
CREATE TABLE "payments" (
	...
	CONSTRAINT "payment_refunded_minor_valid" CHECK ("payments"."refunded_minor" >= 0 and "payments"."refunded_minor" <= "payments"."amount_minor"),
...
--> statement-breakpoint
CREATE TABLE "commerce_applied_events" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"environment" text NOT NULL,
	"provider_event_id" text NOT NULL,
	"event_type" text NOT NULL,
...
CREATE UNIQUE INDEX "commerce_applied_event_environment_provider_uq" ON "commerce_applied_events" USING btree ("environment","provider_event_id");