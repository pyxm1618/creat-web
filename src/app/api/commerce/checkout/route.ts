...
function conflict(error: unknown): boolean {
  return error instanceof Error && (error.message === "checkout initialization in progress" || error.message === "checkout already created");
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (product.commercialModel === "one_time" && !featuresConfig.commerce.oneTime) return new Response("Not Found", { status: 404 });
    if (product.commercialModel === "subscription" && !featuresConfig.commerce.subscriptions) return new Response("Not Found", { status: 404 });
    ...
    if (error instanceof Error && (error.message.includes("product") || error.message.includes("idempotency"))) return Response.json({ error: "invalid_checkout_request" }, { status: 400 });
