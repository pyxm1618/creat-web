...
- The same file carries E2E profiles selected by env. `APP_ENV=test` + `CREAT_WEB_E2E_ENABLED_FEATURES=1` selects the enabled test profile: `auth.enabled=true`, `auth.google=false`, `auth.magicLink=true`, `auth.password=false`, `email.enabled=true`, `commerce.enabled=false`, `commerce.oneTime=false`, `commerce.subscriptions=false`, `commerce.credits=false`, `analytics.enabled=true`, `analytics.ga4=true`, `analytics.clarity=true`, and `analytics.consentRequired=true`. Adding `CREAT_WEB_E2E_COMMERCE=1` selects the commerce-enabled test profile: `commerce.enabled=true`, `commerce.oneTime=false`, `commerce.subscriptions=true`, and `commerce.credits=true`; the auth, email, and analytics values remain those of the enabled test profile. `test:build-matrix` rewrites this file temporarily to build both extremes, so keep it mechanically rewritable (plain object literals).
