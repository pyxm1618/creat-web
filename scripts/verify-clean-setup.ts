...
const expectedModified = new Set([
...
function gitStatus(cwd: string): string[] {
  const output = execFileSync("git", ["status", "--porcelain"], { cwd, encoding: "utf8" }).trim();
  if (!output) return [];
  return output
    .split("\n")
    .filter(Boolean)
...
function assertOnlyExpectedConfigChanges(cwd: string): void {
  const actual = new Set(gitStatus(cwd));
  if (actual.size !== expectedModified.size) {
    throw new Error(`clean setup changed undocumented paths: ${[...actual].join(", ")}`);
  }
  for (const path of expectedModified) {
    if (!actual.has(path)) throw new Error(`clean setup did not apply expected config: ${path}`);
  }
}
...
try {
  ...
  assertOnlyExpectedConfigChanges(checkout);

  const forbidden = spawnSync(
    "git",
    ["grep", "-n", "-i", "-E", "quick[ -]?i[ -]?ching|ichingcoin|hexagram|casting", "--", "src", "examples/neutral-product"],
