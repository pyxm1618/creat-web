...
if (
  !Number.isInteger(subscriptionsConfig.pastDueGraceDays) ||
  subscriptionsConfig.pastDueGraceDays < 0
) {
  throw new Error("subscription grace days must be a nonnegative integer");
}
if (!subscriptionsConfig.gracePolicyVersion.trim())
  throw new Error("subscription grace policy must be versioned");
for (const route of [
  "/api/commerce/subscription/cancel",
  "/api/commerce/subscription/resume",
  "/api/commerce/refunds",
]) {
  if (routeRegistry.get(route).class !== "system")
    throw new Error(`commerce command route must be system-only: ${route}`);
}
