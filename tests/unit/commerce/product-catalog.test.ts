...
it("returns the highest enabled version as an immutable snapshot", () => {
  const catalog = createProductCatalog([
    product,
    { ...product, version: 2, expectedPrice: "3.99", providerProductIdByEnvironment: { test: "prod_test_2" } },
  ]);
...
it("rejects duplicate key/version pairs", () => {
  expect(() => createProductCatalog([product, { ...product }])).toThrow("duplicate product version");
});

it("rejects disabled checkout and missing environment mappings", () => {
  expect(() => createProductCatalog([{ ...product, enabled: false }]).getEnabled("starter-pack", "test"))
    .toThrow("unknown or disabled product");
  expect(() => createProductCatalog([product]).getEnabled("starter-pack", "production")).not.toThrow();
  expect(() =>
    createProductCatalog([{ ...product, providerProductIdByEnvironment: { test: "prod_test_1" } }]).getEnabled(
      "starter-pack",
      "production",
    ),
